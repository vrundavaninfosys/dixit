                            ...
                            <div class="table-responsive mb-4 mt-4">
                                <table id="default-ordering" class="table table-hover style-2" style="width:100%">
                                    ...
                                    <?php
                                        ...
                                        $sql = "select contacts.*,services.name as service from contacts JOIN services ON services.id=contacts.service_id order by contacts.id desc";
                                        ...
                                        if ($result = $conn->query($sql)) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                $contact_id = $row['id'];
                                                echo "<tr><td>" . $i . "</td>";
                                                echo "<td>" . strip_tags($row['fname']) . "</td>";
                                                echo "<td>" . strip_tags($row['lname']) . "</td>";
                                                echo "<td>" . strip_tags($row['email']) . "</td>";
                                                echo "<td>" . strip_tags($row['service']) . "</td>";
                                                echo "<td>" . strip_tags($row['message']) . "</td>";
                                                echo "<td>" . ucfirst($row['status']) . "</td>";
                                                echo "<td>";
                                                echo '<a href="'.url("edit-contact.php?contact_id=$contact_id") .'"><button class="btn btn-success">View</button></a>';
                                                echo '<a href="'.url("contact.php?submit=delete&contact_id=$contact_id") .'" onclick="return confirm(\'Are You Sure ?\')"><button class="btn btn-danger">Delete</button></a>';
                                                echo "</td></tr>\n";
                                                echo "</tr>";
                                                $i++;
                                            }
